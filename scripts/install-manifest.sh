...
# Copies host/ to ~/.local/share/grove-host, links ~/.local/bin/grove-host to it, writes the Brave
...
SHARE="$HOME/.local/share/grove-host"
BIN="$HOME/.local/bin/grove-host"
mkdir -p "$SHARE" "$(dirname "$BIN")"
cp host/prompts.py host/validate.py host/emoji.txt "$SHARE/"
sed -e "1s|.*|#!$PYTHON|" -e "s|^ALLOWED_ORIGIN = \".*\"|ALLOWED_ORIGIN = \"chrome-extension://$ID/\"|" host/grove-host.py > "$SHARE/grove-host.py"
chmod 755 "$SHARE/grove-host.py"
ln -sf "$SHARE/grove-host.py" "$BIN"
echo "host:     $BIN -> $SHARE/grove-host.py"

for ch in "${CHANNELS[@]}"; do
  DIR="$HOME/Library/Application Support/BraveSoftware/$ch/NativeMessagingHosts"
  mkdir -p "$DIR"
  "$BIN" --print-manifest "$BIN" > "$DIR/io.grove.host.json"
  echo "manifest: $DIR/io.grove.host.json"
done
